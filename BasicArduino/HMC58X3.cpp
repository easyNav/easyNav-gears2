...
void HMC58X3::setScale(float f0, float f1, float f2)
{
  x_scale *= f0;
  y_scale *= f1;
  z_scale *= f2;
...
void HMC58X3::setOffsets(int x, int y, int z)
{
   x_off = x;
   y_off = y;
   z_off = z;
...
bool HMC58X3::calibrate(unsigned char gain,unsigned int n_samples) 
{
  ...
  long int xyz_total[3]={
    0,0,0  };  // 32 bit totals so they won't overflow.
  bool bret=true;                 // Function return value.  Will return false if the wrong identifier is returned, saturation is detected or response
//  is out of range to self test bias.
...
#ifdef HMC_COMPENSATE  
  float x0 = (float) xr / x_scale;
  float y0 = (float) yr / y_scale;
  float z0 = (float) zr / z_scale;
