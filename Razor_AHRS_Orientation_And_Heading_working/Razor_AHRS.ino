...
float sensitivity = 0.004;   // as given in datasheet

#define SAMPLE_COUNT 30

// Magnetometer (standard calibration mode)
// "magn x,y,z (min/max) = X_MIN/X_MAX  Y_MIN/Y_MAX  Z_MIN/Z_MAX"
#define MAGN_X_MIN ((float) -49)
#define MAGN_X_MAX ((float) 115)
#define MAGN_Y_MIN ((float) -98)
#define MAGN_Y_MAX ((float) 58)
#define MAGN_Z_MIN ((float) -66)
#define MAGN_Z_MAX ((float) 98)
...
#define GYRO_AVERAGE_OFFSET_X ((float) -32.11)
#define GYRO_AVERAGE_OFFSET_Y ((float) 40.83)
#define GYRO_AVERAGE_OFFSET_Z ((float) -2.02)
...
float accelRealY;
float accelRealZ;
float offsetXLinearAccel;
float offsetOrientation;

float currOrientationFinal=0.0f;
int firstIteration = 1;


float forwardAccel = 0.0f;
float accumAccelOffset = 0.0f;
...
void reset_sensor_fusion() {
  ...
  init_quaternion_vec(yaw,pitch, roll);
}


void findAccelOffset() {

  if(firstIteration <= 300) {

    accumAccelOffset += accel[0];
  }

  if(firstIteration == 301) {

    accelOffset =  accumAccelOffset/301;
  }
...
bool on_ground(){
  ...
  if (voltage > 2.0 || sensorValue > 570){
    return true;
  }
  else{
    return false;
