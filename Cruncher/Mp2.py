...
class CrunchClass:
    ...
    def offset_multiply(self,offset,mult,arr):
        offset_arr = []
        for i in xrange(0, len(arr)):
            offset_arr.append( (arr[i]-offset)*mult )
        return offset_arr
    ...
    def process(self):
        ...
        if len(r_arr) > 10:
            ...
            filtered = butter_lowpass_filter(r_arr,2, 30, 4)

            """
            Integral
            """
            vel_arr = self.integrate(filtered, m_arr)
            dist_arr = self.integrate(vel_arr, m_arr)

            total_sum = self.sumArr(dist_arr)
            print "DIST: "+str(total_sum)
            ...
            return DataClass(raw=r_arr, kal=filtered, smth=filtered, ang=a_arr, vel=vel_arr, dist=dist_arr, ms=m_arr, total=total_sum)

        else:
            #print "DISCARD"
            return None

    def add(self, data, ms, ang, offset):

        elapsed_time = time.time() - self.curr_time
        ms = elapsed_time
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        crunch.curr_time = time.time()
        serialAccel.read()

        # 
        if(serialAccel.on_ground == 0):
            crunch.add(serialAccel.gx, serialAccel.ms, ns.yaw, offset)
        else:
            offset = serialAccel.gx
